...
import {
  getAdminApiEndpoint, getLimit, handleFilterChange, onTableStateChange,
} from '../../../utils/helpers';
...
class AllEvents extends React.Component {
  ...
  fashionData(data) {
    const fashioned = data.map((d) => [d.id, getHumanFriendlyDate(d.start_date_and_time, true), {
      id: d.id,
      image: d.image,
      initials: `${d.name && d.name.substring(0, 2)
        .toUpperCase()}`,
    }, smartString(d.name), d, `${smartString(d.tags.map((t) => t.name)
      .join(', '), 30) // limit to first 30 chars
  ...
  getColumns() {
    ...
    return [{
      ...
      options: {
        filter: false,
        customBodyRender: (d) => (<EventSharedState
          {...d || {}}
          classes={classes}
      ...
      name: 'Community',
      key: 'community',
      options: auth?.is_super_admin ? CustomOptions({
        data: communities,
        label: 'community',
      ...
      options: {
        filter: false,
        download: false,
        customBodyRender: (d) => (<MEChip
          onClick={() => this.props.toggleLive({
            show: true,
      ...
      options: {
        ...
        download: false,
        sort: false,
        customBodyRender: (content) => (<div style={{ display: 'flex' }}>
  ...
  addEventToHomePage = (id) => {
    ...
    apiCall('home_page_settings.addEvent', toSend)
      .then((res) => {
        if (res.success) {
  ...
  customSort(data, colIndex, order) {
    ...
    return data.sort((a, b) => ourCustomSort({
      ...params,
      a,
      b
    }));
  ...
  render() {
    ...
    const options = {
      ...
      confirmFilters: true,
      onTableChange: (action, tableState) => onTableStateChange({
        action,
        tableState,
        tableData: data,
        metaData,
      ...
      customSearchRender: (searchText, handleSearch, hideSearch) => (<SearchBar
        ...
        meta={meta}
      />),
      customFilterDialogFooter: (currentFilterList, applyFilters) => (<ApplyFilterButton
        url={getAdminApiEndpoint(auth, '/events')}
        reduxItems={allEvents}
        updateReduxFunction={putEventsInRedux}
        columns={columns}
        ...
        meta={meta}
      />),
      whenFilterChanges: (changedColumn, filterList, type,) => handleFilterChange({
        filterList,
        type,
        columns,
      ...
      onRowsDelete: (rowsDeleted) => {
        ...
        this.props.toggleModal({
          ...
          onConfirm: () => noTemplatesSelectedGoAhead && this.nowDelete({
            idsToDelete,
            data
          }),
          closeAfterConfirmation: true,
...
const EventSharedState = (props) => {
  ...
  const notSharedYetDialog = () => {
    toggleModalOnClick && toggleModalOnClick({
      ...
      component: (<EventNotSharedWithAnyone
        publicity={publicity}
  ...
  const sharedWithDialog = () => {
    toggleModalOnClick && toggleModalOnClick({
      show: true,
      title: smartString(name, 50),
      component: (<EventSharedWithCommunity
        publicity={publicity}
        shared_to={shared_to}
        id={id}
        shareable_to={communities_under_publicity}
