...
class TestimonialsPageEditForm extends Component {
  constructor(props) {
    super(props);
    this.state = {
      formJson: null,
  ...
  async componentDidMount() {
    ...
    const pageData = await apiCall('/testimonials_page_settings.info', { community_id: id });
  ...
  createFormJson = async () => {
    ...
    const formJson = {
      title: `Edit ${community ? community.name + '\'s' : 'Community\'s'} - All Testimonials Page`,
      subTitle: '',
      method: '/testimonials_page_settings.update',
  ...
  render() {
    ...
    return (
      <div>
        <Seo name={`Edit - ${pageData?.community?.name}'s Testimonials Page`} />
        <MassEnergizeForm
          classes={classes}
          formJson={formJson}
        />
...
TestimonialsPageEditForm.propTypes = {
  classes: PropTypes.object.isRequired,
};


export default withStyles(styles, { withTheme: true })(TestimonialsPageEditForm);
